...
export class EcsFieldsetComponent extends EditableEntityComponent<EcsFieldset> implements OnInit, OnChanges {
  ...
  @Input() parameterDescList: ParameterDescription[] | null | undefined;
  ...
  override ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['parameterDescriptions']?.currentValue) {
      ...
      const fieldsetParameters: ParameterDescription[] | null  | undefined = this.parameterDescList?.filter(
        (p) => p.type === 'fieldset'
      );
      fieldsetParameters?.forEach((p) => {
        this.fieldsetParameterDescriptions.set(p.parameterName, p.description);
      });
      const fieldParameters: ParameterDescription[] | null  | undefined = this.parameterDescList?.filter((p) => p.type === 'field');
